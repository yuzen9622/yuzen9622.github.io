...
const opacity = useTransform(scrollYProgress, [0, 0.5], [0, 1]);
  return (
    ...
    id="contact"
      ...
      className="relative  w-11/12 bg-background py-20 "
      style={{ opacity }}
    >
      ...
      <div className="grid grid-cols-2 gap-6 lg:w-8/12  place-content-center">
        <div className=" lg:text-xl text-base  space-y-3 flex-wrap">
          <h2 className=" text-primary bbh-sans-bartle-regular">[Contact]</h2>
          <a className=" inter" href={`tel:${profile.phone}`}>{profile.phone}</a>
        </div>
        <div className=" lg:text-xl text-base space-y-3">
          <h2 className=" text-primary bbh-sans-bartle-regular">[Email]</h2>
         <a className=" inter" href={`mailto:${profile.email}`}>{profile.email}</a>
        </div>
        <div className=" lg:text-xl text-base space-y-3">
